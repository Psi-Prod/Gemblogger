...
let get_article (module V : Metadata.VALIDABLE) article_file =
  let arr =
    Build.read_file_with_metadata (module V) (module Model.Article) article_file
  in
...
let get_articles (module V : Metadata.VALIDABLE) path =
  let* files = read_child_files path File.is_gemtext in
...
module Tags = struct
  ...
  let group metas =
    ...
    |> M.to_seq |> List.of_seq |> List.sort by_quantity
